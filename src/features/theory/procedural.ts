...
async function getMeasuresForChord(
  ...
  let filename = `${import.meta.env.BASE_URL}music/irish/Right Hand/${chordAndHighOrLow}_Lvl_${level}.mid`
  if (clef === 'bass') {
    const chord = chordAndHighOrLow.slice(0, chordAndHighOrLow.indexOf('_'))
    const bassLevel = Math.min(2, level)
    filename = `${import.meta.env.BASE_URL}music/irish/Left Hand/${chord.toString()}_Lvl_${bassLevel}_LH.mid`
  }
